...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        ...
        if bg_entities:
            if isinstance(bg_entities, list):
                self.entity_list.extend([e for e in bg_entities if e is not None])
            else:
    ...
    def run(self, player_score: list[int]):
        ...
        while True:
            ...
            # Desenho e movimentação
            for ent in list(self.entity_list):  # cópia para evitar problemas ao remover
                if ent is None:
                    continue
                self.window.blit(source=ent.surf, dest=ent.rect)
                ent.move()
            ...
            for event in pygame.event.get():
                ...
                # Se não houver jogadores vivos, fim de jogo
                found_player = any(isinstance(ent, Player) for ent in self.entity_list if ent is not None)
                if not found_player:
                    return False
