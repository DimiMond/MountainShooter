...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == EVENT_ENEMY:
            choice = random.choice(('Enemy1', 'Enemy2'))
            new_entities.append(EntityFactory.get_entity(choice))
        if event.type == EVENT_TIMEOUT:
            self.timeout -= TIMEOUT_STEP
            if self.timeout == 0:
                for ent in self.entity_list:
    ...
    # colisões e remoções antes do desenho
    EntityMediator.verify_collision(entity_list=self.entity_list)
    EntityMediator.verify_health(entity_list=self.entity_list)
    ...
    # verifica se não houver jogador
    found_player = any(isinstance(ent, Player) for ent in self.entity_list)
    if not found_player:
        return False

    # desenho
    self.window.fill((0, 0, 0))  # fundo (ou desenhe seu BG antes)
    for ent in self.entity_list:
        self.window.blit(source=ent.surf, dest=ent.rect)
        if ent.name == 'Player1':
            self.level_text(14, f'Player1 - Health: {ent.health} | Score: {ent.score}', C_GREEN, (10, 25))
